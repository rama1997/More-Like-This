...
const { createMetaPreview } = require("./metaPreview");
...
async function generateCatalogSource(id, apiKeys, userConfig, metadataSource) {
...
async function resolveSearchInput(searchKey, searchYear, type, metadataSource) {
	...
	if (searchKey.startsWith("tt")) {
		...
		title = searchKey;
		year = searchYear;
		imdbId = await idConverter.titleToImdb(searchKey, searchYear, type, metadataSource);
	}
...
async function catalogHandler(type, id, extra, userConfig, metadataSource) {
	...
	let catalogSource = await generateCatalogSource(id, apiKeys, userConfig, metadataSource);
	...
	if (searchImdb) {
		...
		let cachedRecsCatalog = await checkCache(searchImdb, null, type, catalogSource);
	...
	if (!recs || recs.length === 0) {
		...
		catalog = await createRecCatalog(recs, type, apiKeys, metadataSource);
		...
		// Save to cache
		await saveCache(searchImdb, null, type, catalogSource, catalog);
